...
const RACES = ['Human', 'Elf', 'Dwarf', 'Halfling', 'Gnome', 'Half-Orc', 'Tiefling', 'Dragonborn', 'Half-Elf', 'Other'];
...
export default function WizardForm({ mode }: WizardFormProps) {
  ...
  const handleNext = () => {
    ...
    if (step === 1 && !race) return;
    if (step === 2 && !charClass) return;
    if (step === 3 && !personality) return;
  ...
  const generateFinalCharacter = async () => {
    ...
    const backstory = generateBackstory(name, race, charClass, personality);
    const portraitUrl = generatePortraitUrl(race, charClass, personality);
          ...
          <div className="grid grid-cols-2 gap-3 pb-8">
            {RACES.map(r => (
              <button
                ...
                onClick={() => setRace(r)}
                className={cn(
                  "px-4 py-4 rounded-xl border-2 font-display tracking-wider text-sm transition-all duration-300 hover-elevate",
                  race === r 
                    ? "bg-primary/20 border-primary text-primary text-glow shadow-[0_0_15px_rgba(234,179,8,0.2)]" 
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[60vh]">
        ...
        >
          ...
          <div className="absolute inset-4 border-4 border-t-accent/20 border-r-accent/20 border-b-accent border-l-accent rounded-full animation-reverse" />
          <Sparkles className="absolute inset-0 m-auto w-12 h-12 text-primary animate-pulse" />
        </motion.div>
        <h2 className="text-3xl font-display font-bold text-primary text-glow mb-4">Weaving Fates...</h2>
        <p className="text-muted-foreground text-lg italic">Consulting the ancient tomes and conjuring visions of {name || 'the unknown'}...</p>
      </div>
    );
  }
